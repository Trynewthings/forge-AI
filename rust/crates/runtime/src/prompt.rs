...
pub struct SystemPromptBuilder {
    ...
    /// Active model id (e.g. `claude-opus-4-7`, `deepseek-v4-flash`). When
    /// set, the `Model family:` line in the environment section reports this
    /// instead of the legacy compile-time fallback, so the LLM can answer
    /// "what model are you?" truthfully and can size its own context use.
    model: Option<String>,
}
...
impl SystemPromptBuilder {
    ...
    pub fn build(&self) -> Vec<String> {
        ...
        sections.push(get_when_to_ask_section());
        sections.push(get_long_session_section());
    ...
    fn environment_section(&self) -> String {
        ...
        let model = self.model.as_deref().unwrap_or(FRONTIER_MODEL_NAME);
...
fn get_simple_doing_tasks_section() -> String {
    let items = prepend_bullets(vec![
        ...
        // Added: requirement coverage check. Concrete benchmark observation —
        // the agent landed backend + tests but skipped the frontend deliverable
        // because nothing forced a final pass over the original request.
        "Before declaring a task complete, re-read the user's request and verify each explicit requirement was addressed. Surface anything skipped or deferred.".to_string(),
        // Added: planning. Agent has TodoWrite but doesn't reliably reach for
        // it on medium tasks. Make the trigger explicit.
        "Use the TodoWrite tool to plan any task with 3 or more steps and update it as you progress. Skip TodoWrite for trivial single-step changes.".to_string(),
        // Added: test sizing. Caps the temptation to write 30 near-duplicate
        // cases when 5 representative ones would cover the same risk.
        "Tests should cover representative cases and important branches — not every permutation. Match the existing test density of the codebase.".to_string(),
    ]);
...
/// Concrete channel-routing rules for the built-in tools. Empirically
/// the model wastes turns by reaching for `bash cat ...` instead of
/// `read_file`, or by `write_file`-ing an existing file just to change
/// three lines. The descriptions in the `tools` array tell the model
/// *what each tool does*; this section tells it *which to pick and
/// what NOT to do* — that's a different axis and worth stating in
/// natural language up front.
fn get_tool_usage_section() -> String {
    let items = prepend_bullets(vec![
        "Use read_file (not `bash cat / head / sed`) to inspect files — it returns line numbers and supports offset/limit for large files.".to_string(),
        "Use bash for shell operations: ls, rg/grep, find, build commands, tests, git, package managers.".to_string(),
        "Use edit_file for any change to an existing file. Use write_file ONLY for brand-new files or full rewrites — never to make small edits.".to_string(),
        "edit_file `old_string` must match the file's current content EXACTLY, including indentation and trailing whitespace. If a match fails, re-read the file before retrying — do not guess.".to_string(),
        "For multiple disjoint changes in one file, make sequential edit_file calls; do not rewrite the whole file via write_file.".to_string(),
        "Long-running bash commands: pass `timeout` (ms) or `run_in_background: true` so the turn doesn't block. For commands with huge output (large logs, multi-MB dumps), redirect to a file (`cmd > /tmp/out.log`) and then read_file it with offset/limit — pasting megabytes of text into the conversation wastes context fast.".to_string(),
        "Be concise in your prose responses. Show file paths verbatim. Don't restate what tool calls already revealed.".to_string(),
    ]);

    std::iter::once("# Tool usage".to_string())
        .chain(items)
        .collect::<Vec<_>>()
        .join("\n")
}

/// When to use AskUser. Earlier version of this section framed it as
/// "ask sparingly" and the model latched onto "just pick a reasonable
/// default" as self-permission to skip even high-stakes decisions
/// (e.g. new field validation rules). Reframed to lead with positive
/// triggers — the model should ask when a request hides product
/// judgement, not when it's "100% sure intent is unclear".
fn get_when_to_ask_section() -> String {
    let items = prepend_bullets(vec![
        "Requests that touch persistent state (new fields, schema migrations, validation rules, defaults) almost always hide product decisions the user should own. Before doing the work, list the assumptions you'd otherwise guess (is this field required? unique? what format? null vs empty default?). If there's more than one substantive guess, AskUser ONCE up front instead.".to_string(),
        "AskUser before destructive operations (delete, drop, reset, force-push, rewriting history) that weren't explicitly authorized in the user's message.".to_string(),
        "AskUser when picking between architecturally different approaches with comparable merit (sync vs async API, SQLite vs Postgres, etc.) — these become long-lived constraints.".to_string(),
        "Do NOT AskUser for naming, code style, helper function picks, comment phrasing, or other cosmetic choices — follow the local convention and move on.".to_string(),
        "When asking, give 2-4 concrete `options` with one-line `description` each. Constrained choices yield cleaner answers than free-text questions.".to_string(),
        "If AskUser returns `dismissed: true`, do NOT re-ask — pick a sensible default and proceed. The user has implicitly delegated the call to you.".to_string(),
    ]);

    std::iter::once("# When to ask the user".to_string())
        .chain(items)
        .collect::<Vec<_>>()
        .join("\n")
}

/// Habits that keep mid-to-long sessions tractable. Mostly nudges the
/// model away from re-reading the same files and from speculative
/// rewrites that blow up the context window with churn.
fn get_long_session_section() -> String {
    let items = prepend_bullets(vec![
        "Prefer focused edits over wide rewrites; less context churn means clearer reasoning later in the session.".to_string(),
        "When context is heavily used, summarise prior findings instead of re-reading the same file.".to_string(),
    ]);

    std::iter::once("# Working in long sessions".to_string())
        .chain(items)
        .collect::<Vec<_>>()
        .join("\n")
}
