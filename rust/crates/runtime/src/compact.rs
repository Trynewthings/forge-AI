...
fn summarize_block(block: &ContentBlock) -> String {
    let raw = match block {
        ...
        ContentBlock::Reasoning { text, .. } => format!("[thinking] {text}"),
...
fn first_text_block(message: &ConversationMessage) -> Option<&str> {
    message.blocks.iter().find_map(|block| match block {
        ...
        // Reasoning is the model's internal monologue, not a useful
        // "current work" summary — skip it when scanning for the
        // headline text of a message.
