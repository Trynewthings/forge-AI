...
/// Result of a full discovery sweep. `tools` are all the tools that came
/// back successfully (across every healthy server); `failures` are servers
/// whose `initialize` or `tools/list` errored. The two never overlap — a
/// server appears in one or the other, never both.
#[derive(Debug)]
pub struct McpDiscoveryReport {
    pub tools: Vec<ManagedMcpTool>,
    pub failures: Vec<McpServerDiscoveryFailure>,
}

#[derive(Debug)]
pub struct McpServerDiscoveryFailure {
    pub server_name: String,
    pub error: McpServerManagerError,
}
...
impl McpServerManager {
    ...
    /// Discover tools across every configured server, isolating failures per
    /// server so one broken MCP doesn't take down the rest.
    ///
    /// Previously this returned `Result<Vec<_>>` and bailed on the first
    /// per-server error — losing every tool already discovered from earlier
    /// servers. The new shape collects partial results: successful tools go
    /// into `tools`, failures go into `failures` so callers (and the UI) can
    /// report which server is sick without affecting healthy ones.
    pub async fn discover_tools(&mut self) -> McpDiscoveryReport {
        let server_names = self.servers.keys().cloned().collect::<Vec<_>>();
        let mut discovered_tools = Vec::new();
        let mut failures = Vec::new();

        for server_name in server_names {
            match self.discover_tools_for(&server_name).await {
                Ok(mut tools) => discovered_tools.append(&mut tools),
                Err(error) => failures.push(McpServerDiscoveryFailure {
                    server_name,
                    error,
                }),
            }
    ...
    /// Per-server discovery — extracted so the outer loop's error handling
    /// stays linear. Routes are cleared up-front so a failure here doesn't
    /// leave a stale `tool_index` entry pointing at a server we couldn't
    /// reach.
    async fn discover_tools_for(
        &mut self,
        server_name: &str,
    ...
    async fn ensure_server_ready(
        ...
        if needs_initialize {
            ...
            // Send the `notifications/initialized` notification so the server
            // knows our handshake is done and starts replying to subsequent
            // requests like tools/list. Several reference MCP servers will
            // otherwise sit on tools/list forever.
            {
                let server = self.server_mut(server_name)?;
                let process = server.process.as_mut().ok_or_else(|| {
                    McpServerManagerError::InvalidResponse {
                ...
                process
                    .write_jsonrpc_message(&notification)
                    .await
...
impl McpStdioProcess {
    ...
    pub async fn write_jsonrpc_message<T: Serialize>(&mut self, message: &T) -> io::Result<()> {
        // MCP stdio is line-delimited JSON, NOT LSP-style Content-Length framing.
        // We keep `write_frame` for the few tests that still drive the LSP-style
        // framing, but real MCP servers (the reference impls) talk JSON-per-line.
    ...
    pub async fn read_jsonrpc_message<T: DeserializeOwned>(&mut self) -> io::Result<T> {
        // Line-delimited JSON over MCP stdio. Skip blank lines (some servers emit
    ...
    /// Read frames until we find a response matching the given request id. Server
    /// notifications and unrelated responses are silently dropped — they're allowed
    /// to interleave with our request/response pairs per MCP spec.
    pub async fn read_response_for_id<T: DeserializeOwned>(
        &mut self,
        expected: &JsonRpcId,
    ) -> io::Result<JsonRpcResponse<T>> {
        loop {
            let envelope: serde_json::Value = self.read_jsonrpc_message().await?;
            // Notifications and server-side requests have a `method` field but no
            // `id` matching our request — skip them.
            ...
            let response: JsonRpcResponse<T> = serde_json::from_value(envelope)
                .map_err(|error| io::Error::new(io::ErrorKind::InvalidData, error))?;
            if &response.id == expected {
                return Ok(response);
            }
            // Response for an unrelated request id — also drop and keep reading.
        }
    }
...
mod tests {
    ...
    fn write_jsonrpc_script() -> PathBuf {
        ...
        // Line-delimited JSON to match MCP stdio wire format.
        let script = [
            "#!/usr/bin/env python3",
            "import json, sys",
    ...
    fn write_mcp_server_script() -> PathBuf {
        ...
        // Line-delimited JSON to match the real MCP stdio wire format.
        let script = [
            "#!/usr/bin/env python3",
            "import json, sys",
            "",
    ...
    /// Regression: a single broken MCP server used to take down discovery
    /// for every other configured server (the `?`-propagating loop in
    /// `discover_tools`). After the per-server-resilience refactor, the
    /// healthy server's tools must still come back, and the broken one
    /// must surface in `failures`.
    #[test]
    fn manager_discovery_isolates_per_server_failures() {
        let runtime = Builder::new_current_thread()
            .enable_all()
            .build()
            .expect("runtime");
        runtime.block_on(async {
            let script_path = write_manager_mcp_server_script();
            let root = script_path.parent().expect("script parent");
            let good_log = root.join("good.log");
            let broken = ScopedMcpServerConfig {
                scope: ConfigSource::Local,
                config: McpServerConfig::Stdio(McpStdioServerConfig {
                    command: "/definitely/not/a/real/binary-xkcd".to_string(),
                    args: vec![],
                    env: BTreeMap::new(),
                }),
            };
            let servers = BTreeMap::from([
                (
                    "good".to_string(),
                    manager_server_config(&script_path, "good", &good_log),
                ),
            ...
            // Healthy server's tool still came through.
            assert_eq!(report.tools.len(), 1, "good server's tool must survive");
            assert_eq!(report.tools[0].server_name, "good");

            // Broken server is reported as a failure, not silently swallowed.
            assert_eq!(report.failures.len(), 1, "broken server must be reported");
            assert_eq!(report.failures[0].server_name, "broken");
