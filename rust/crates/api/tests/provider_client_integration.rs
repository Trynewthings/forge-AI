...
fn provider_client_routes_deepseek_aliases_through_deepseek() {
    ...
    let client = ProviderClient::from_model("deepseek").expect("deepseek alias should resolve");
...
#[test]
fn provider_client_falls_back_to_generic_openai_compat_env_for_unknown_models() {
    let _lock = env_lock();
    ...
    let _deepseek_api_key = EnvVarGuard::set("DEEPSEEK_API_KEY", None);
    let _xai_api_key = EnvVarGuard::set("XAI_API_KEY", None);
    let _generic_api_key = EnvVarGuard::set("OPENAI_COMPAT_API_KEY", Some("compat-test-key"));

    let client = ProviderClient::from_model("custom-local-model")
        .expect("generic OpenAI-compatible provider should resolve");
