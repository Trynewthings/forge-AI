...
fn normalize_allowed_tools(values: &[String]) -> Result<Option<AllowedToolSet>, String> {
    ...
    let builtin_registry = GlobalToolRegistry::builtin();
    match builtin_registry.normalize_allowed_tools(values) {
        Ok(allowed_tools) => Ok(allowed_tools),
        Err(builtin_error) => match current_tool_registry() {
            Ok(tool_registry) => tool_registry.normalize_allowed_tools(values),
            Err(_) => Err(builtin_error),
        },
    }
}
